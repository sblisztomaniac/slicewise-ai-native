...
const AddFundingRound: FC<AddFundingRoundProps> = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // The round was already added to the context, just update the current round for display
      const newRound: FundingRound = {
        ...formData,
        id: generateId(),
        date: new Date(formData.date).toISOString(),
      ...
      // Don't reset the form, keep the values for reference
    } catch (error) {
  ...
  const handleStyleToggle = async (style: '12yo' | 'mentor' | 'expert') => {
    ...
    // Update explanation with the new style
    try {
...
// ...
  return (
    ...
    >
      <motion.div
        className="w-full max-w-2xl max-h-[90vh] flex flex-col bg-white rounded-lg shadow-xl overflow-hidden"
        ...
        {explanation && (
          <div className="border-t border-gray-200">
            <motion.div 
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              className="p-4 text-sm bg-blue-50 max-h-96 overflow-y-auto"
            >
                ...
                <div className="mt-3 p-3 bg-white rounded border border-blue-100 text-sm">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div>
                      ...
                      <p className="font-medium capitalize">{currentRound.type}</p>
                    </div>
                    <div>
                      <p className="text-gray-600">Amount</p>
                      <p className="font-medium">${currentRound.amount.toLocaleString()}</p>
                    </div>
                    <div>
                      <p className="text-gray-600">Valuation</p>
                      <p className="font-medium">${currentRound.valuation.toLocaleString()}</p>
                    </div>
                    <div>
                      <p className="text-gray-600">Ownership</p>
                      <p className="font-medium">
                        {currentRound.ownershipPercentage?.toFixed(2)}%
                      </p>
                    </div>
                  </div>
                </div>
